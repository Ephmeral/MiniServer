...
/*网站的根目录*/
const char* doc_root = "/home/ubuntu/MiniServer/root";
...
HTTPConn::HTTP_CODE HTTPConn::parse_request_line(char* text) {
    ...
    *m_url++ = '\0';
    ...
    char* method = text;
    /*仅支持GET方法*/
    // strcasecmp 会逐字节比较，不区分大小写
    if (strcasecmp( method, "GET" ) == 0) {
        m_method = GET;
    } else {
        return BAD_REQUEST;
    }
    // strspn(s, accept)函数用于计算字符串s中开头连续包含了多少个出现在字符集合accept中的字符
    // 这里相当于将开头所有连续的" \t"给清除了
    m_url += strspn( m_url, " \t" );
...
HTTPConn::HTTP_CODE HTTPConn::do_request() {
    ...
    // 将m_url拷贝到doc_root后面，最终请求的文件目录为 doc_root + m_url
    strncpy( m_real_file + len, m_url, FILENAME_LEN - len - 1 );
    // 查看文件信息，如果失败，说明请求资源文件失败
    if ( stat( m_real_file, &m_file_stat ) < 0 ) {
        return NO_RESOURCE;
    }
    // S_IROTH标志位来确定其他用户是否可读取该文件
    // 如果其他用户不可读，则说明禁止访问，最后应该返回404？
    ...
    // 前面检查都通过，说明此时请求的是一个文件名正确的，且其他用户可访问的文件
    // 打开这个文件，并通过mmap映射到m_file_address中
